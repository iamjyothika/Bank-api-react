...
class ListCustomer(APIView):
    ...
    def get(self,request):
        if request.method == 'GET':
            customer = Customer.objects.exclude(is_superuser=True)
...
class CustomerDetailsAPIView(APIView):
    ...
    def get(self, request):
        try:
            ...
            transactions = Transaction.objects.filter(account=account)  # Assuming transactions are linked to Account
        except Customer.DoesNotExist:
            return Response({"detail": "Customer not found."}, status=status.HTTP_404_NOT_FOUND)
        except Account.DoesNotExist:
            return Response({"detail": "Account not found."}, status=status.HTTP_404_NOT_FOUND)

        # Serialize customer, account, and transactions data
        customer_serializer = CustomerSerializer(customer)
        account_serializer = AccountSerializer(account)
        transaction_serializer = TransactionSerializer(transactions, many=True)
        
        # Prepare the response data including account_no
        response_data = {
            ...
            'balance': account_serializer.data['balance'],  # Use balance from account_serializer
            'transactions': transaction_serializer.data  # List of transactions
        }
...
class DashboardOverviewAPIView(APIView):
    ...
    def get(self, request):
        ...
        recent_activities = Loan.objects.order_by('-created_at')[:5]
...
class SearchFilterUsersView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        query = request.query_params.get('q', '')
        users = Customer.objects.filter(username__icontains=query)
